...
impl FromTOMLValue for String {
    fn from_toml_value(value: &Value) -> Result<Self, String> {
        match *value {
            Value::String(ref string) => Ok(string.clone()),
...
impl FromTOMLValue for StorageConfig {
    fn from_toml_value(value: &Value) -> Result<Self, String> {
        Ok(StorageConfig {
            path: parse_table_field(value, "path")
                .map_err(|msg| format!("[storage].path: {}", msg))?,
            listfile: parse_table_field(value, "listfile").ok(),
        })
    }
}
...
impl FromTOMLValue for ExtractConfig {
    fn from_toml_value(value: &Value) -> Result<Self, String> {
        Ok(ExtractConfig {
            globs: parse_table_field(value, "globs")
                .map_err(|msg| format!("[extract].globs: {}", msg))?,
            out_dir: parse_table_field(value, "out_dir")
                .map_err(|msg| format!("[extract].out_dir: {}", msg))?,
        })
    }
}
...
impl Config {
    pub fn parse(src: &str) -> Result<Self, String> {
        ...
        let table = parser.parse().ok_or_else(|| {
            format!(
                "{:?}",
                parser.errors.iter().fold(String::new(), |mut msg, e| {
                    msg.push_str(&e.desc);
                    msg
                })
            )
        ...
        Ok(Config {
            storage: parse_table_field(value, "storage")
                .map_err(|msg| format!("[storage]: {}", msg))?,
            extract: parse_table_field(value, "extract")
                .map_err(|msg| format!("[extract]: {}", msg))?,
        })
    }
}
